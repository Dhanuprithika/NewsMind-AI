...
const NAV_LINKS = ['Markets', 'Business', 'Startups', 'Banking', 'Tech', 'Economy'];

const SUB_NAV = ['Top Stories', 'For You', 'Politics', 'World', 'Commodities', 'IPO', 'MF', 'Real Estate'];
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between h-12 sm:h-14 gap-3">
            ...
            <nav className="hidden lg:flex items-center gap-0.5 flex-1 justify-center">
              {NAV_LINKS.map((link) => (
                <button
                  key={link}
                  className="px-3 py-1.5 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-md transition-colors tracking-wide whitespace-nowrap"
                >
                  {link}
                </button>
              ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {NAV_LINKS.map((link) => (
                      <button
                        key={link}
                        className="px-3.5 py-1.5 text-sm text-gray-700 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-full transition-colors"
                      >
                        {link}
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SUB_NAV.map((item) => (
                      <button
                        key={item}
                        className="px-3 py-1 text-xs text-gray-600 bg-gray-50 hover:bg-[#c0392b] hover:text-white border border-gray-200 hover:border-[#c0392b] rounded-full transition-colors"
                      >
                        {item}
                      </button>
                    ))}
      ...
      {/* ── Section sub-nav ── */}
      <div className="bg-[#faf8f4] border-b border-gray-200">
        <Container>
          <div className="flex items-center gap-0 overflow-x-auto scrollbar-hide">
            {SUB_NAV.map((item, i) => (
              <button
                ...
                className={`shrink-0 px-3 sm:px-4 py-2 text-xs tracking-widest uppercase border-b-2 transition-colors whitespace-nowrap ${
                  activeSubNav === i
